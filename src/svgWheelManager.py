...
def insertCircle(
    ...
    svgData = m.group(1)
    svgData = svgData + "\n<circle cx=\"" + str(centreX) + "\" cy=\"" + str(centreY) + "\" r=\"" + str(
        radius) + "\" fill=\"" + fillColour + "\" stroke=\"" + strokeColour + "\" stroke-width=\"" + str(strokeWidth) + "\" id=\"" + circleID + "\"  />"
    svgData = svgData + "\n" + m.group(2)
    return svgData
...
def insertRect(
    ...
    svgData = svgData + "\n<rect x=\"" + str(leftPosition) + "\" y=\"" + str(topPosition) + "\" height=\"" + str(height) + "\" width=\"" + str(
        width) + "\" fill=\"" + fillColour + "\" stroke=\"" + strokeColour + "\" stroke-width=\"" + str(strokeWidth) + "\" id=\"" + rectID + "\"  />"
    svgData = svgData + "\n" + m.group(2)
    return svgData
...
class SvgWheelManager:
    ...
    def insertCircleOnPath(self, path):
        ...
        if not "path" in pathID:
            ...
            self.inputSVG = insertCircle(
                self.inputSVG,
                cx,
                cy,
                radius=self.defaultCircleRadius(),
                circleID="circle-" +
                pathID)
    ...
    def insertRectAroundPath(self, path):
        ...
        if not "path" in pathID:
            ...
            self.inputSVG = insertRect(
                ...
                rectID="rect-" +
                pathID,
                strokeWidth=self.defaultLineWidth())
    ...
    def insertCircleChartOnPath(self, path, data):
        ...
        fontsize = outerCircleRadius * 0.5
        textOffsetX = outerCircleRadius / 2
        textOffsetY = fontsize / 3
        ...
        X3 = cx + dataCircleRadius * math.sin(alpha)
        Y3 = cy - dataCircleRadius * math.cos(alpha)    # /!\ inverted y axis
        largeArcFlag = 0
        if (alpha > math.pi):
            largeArcFlag = 1
        sweepFlag = 1

        svgCode = "\n<g><circle cx=\"" + str(cx) + "\" cy=\"" + str(cy) + "\" r=\"" + str(outerCircleRadius) + "\" fill=\"" + \
            outerFillColour + "\" stroke=\"" + outerStrokeColour + "\" stroke-width=\"" + str(outerStrokeWidth) + "\" />\n"
        svgCode = svgCode + "<circle cx=\"" + str(cx) + "\" cy=\"" + str(cy) + "\" r=\"" + str(
            innerCircleRadius) + "\" fill=\"" + innerFillColour + "\" stroke=\"" + innerStrokeColour + "\" stroke-width=\"" + str(innerStrokeWidth) + "\" />\n"
        svgCode = svgCode + "<path\n"
        svgCode = svgCode + "\td=\"M " + str(X1) + "," + str(Y1) + "\n"
        svgCode = svgCode + "\tL " + str(X2) + "," + str(Y2) + "\n"
        #svgCode = svgCode + "\tL "+str(X3)+","+str(Y3)+"\n"
        svgCode = svgCode + "\tA " + str(dataCircleRadius) + "," + str(dataCircleRadius) + " 0 " + str(
            largeArcFlag) + "," + str(sweepFlag) + " " + str(X3) + "," + str(Y3) + "\n"
        svgCode = svgCode + "\tZ\"\n"
        #svgCode = svgCode + "fill=\""+dataFillColour+"\" stroke=\""+dataStrokeColour+"\" stroke-width=\""+str(dataStrokeWidth)+"\" id=\"circleChart-"+pathID+"\" label=\"alpha = "+str(alpha)+ "\" />"
        svgCode = svgCode + "fill=\"" + dataFillColour + "\" stroke=\"" + dataStrokeColour + \
            "\" stroke-width=\"" + str(dataStrokeWidth) + "\" id=\"circleChart-" + pathID + "\" /> \n "
        # font-family="Verdana" font-size="55" fill="blue" >
        svgCode = svgCode + "<text x=\"" + str(cx - textOffsetX) + "\" y=\"" + str(
            cy + textOffsetY) + "\" font-size=\"" + str(fontsize) + "\" > "
        #svgCode = svgCode + " Alpha = " + str(alpha) + " Data = " + str(data) + " </text>"
        svgCode = svgCode + str(data) + "% </text>"
        svgCode = svgCode + "</g>"
    ...
    # Fills self.tagsAndPercentage with percentage of total, as in
    # localValue/Sum(localValues)
    def fillTagsAndPercentageOfTotal(self):
        ...
        for i in self.tagsAndAbsolueValues.keys():
            self.tagsAndPercentage[i] = self.tagsAndAbsolueValues[
                i] * 100.0 / fillTagsAndPercentageOfTotal
